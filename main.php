...
if($profession=="начальник отдела"){
    $sql = "SELECT department.chief, worker.ID_Worker ,work.time_start ,work.time_end FROM worker,department,work WHERE chief='$name'";
...
elseif($profession=='разработчик'){
    ...
    $sql= $link->prepare("UPDATE work SET time_start='$value1', time_end='$value2' WHERE FID_Projects=:name");
